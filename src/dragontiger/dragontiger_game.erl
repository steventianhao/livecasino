...
stopped(new_shoe,{Pid,_},State=#state{dealer=DealerNow,round=Round,table=Table})->
	lager:info("stopped#new_shoe,state ~p",[State]),
	Fun1 = fun()->
			NewRound=dragontiger_round:new_shoe(Round),
			NewState=State#state{round=NewRound},
			?NOTIFY(Table,{new_shoe,NewRound}),
			{reply,ok,stopped,NewState}
		end,
	Fun2= fun()-> {reply,error_channel,stopped,State} end,
	checkDealer(DealerNow,Pid,Fun1,Fun2);

stopped(start_bet,{Pid,_},State=#state{countdown=Countdown,dealer=DealerNow,round=Round,table=Table})->
	...
	Fun1 = fun()->
			case Round of
				...
				_ ->
					...
					?NOTIFY(Table,{start_bet,NewRound,Countdown}),
...
betting(stop_bet,{Pid,_},State=#state{ticker=Ticker,dealer=DealerNow,round=Round,table=Table})->
	...
	Fun1 = fun() ->
			...
			?NOTIFY(Table,{stop_bet,NewRound}),
...
dealing(Event={deal,Pos,Card},{Pid,_},State=#state{cards=Cards,dealer=DealerNow,table=Table})->
...
dealing(Event={scan,Card},{Pid,_},State=#state{cards=Cards,dealer=DealerNow,table=Table})->
...
dealing(Event={clear,Pos},{Pid,_},State=#state{cards=Cards,dealer=DealerNow,table=Table})->
	lager:info("dealing#clear, Event ~p, State ~p",[Event,State]),
	Fun1 = fun()->
		case dragontiger_dealer_mod:remove(Pos,Cards) of
			{ok,NewCards} ->
				NewState=State#state{cards=NewCards},
				?NOTIFY(Table,{clear,Pos}),
				{reply,ok,dealing,NewState};
			error ->
				{reply,error,dealing,State}
		end
...
dealing(commit,{Pid,_},State=#state{cards=Cards,dealer=DealerNow,round=Round,table=Table})->
	...
	Fun1 = fun()->
		case baccarat_dealer_mod:validate(Cards) of
			true->
				...
				NewState=State#state{round=NewRound},
				?NOTIFY(Table,{commit,Round,Cards}),				
				{reply,ok,stopped,NewState};
			false->
				{reply,error,dealing,State}
		end
...
handle_info(tick,betting,State=#state{ticker=Ticker,table=Table})->
	...
	case Ticker of
		...
		{TRef,Value}->
			?NOTIFY(Table,{tick,Value}),
			NewState=State#state{ticker={TRef,Value-1}},
			{next_state,betting,NewState}
	end;
handle_info(Info={'DOWN',_Ref,process,Pid,_},StateName,State=#state{dealer=DealerNow,table=Table})->
	...
	case DealerNow of
		{Pid,Dealer}->
			...
			?NOTIFY(Table,{dealer_disconnect,Dealer}),
...
handle_event(Event={dealer_disconnect,Pid},StateName,State=#state{dealer=DealerNow,table=Table})->
...
handle_sync_event(Event={dealer_connect,Dealer},From={Pid,_},StateName,State=#state{dealer=DealerNow,table=Table})->
	...
	{Result,NewState}=case DealerNow of
		undefined->
		   	...
		   	?NOTIFY(Table,{dealer_connect,Dealer}),
			{ok,State2};
		_ ->
